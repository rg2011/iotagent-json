...
function parserBody() {
    ...
    const jsonParser = bodyParser.json({ strict: false }); // accept anything JSON.parse accepts.
    const xmlParser = bodyParser.xml({
        // The "type" option should return true if the body should be parsed.
        // by default it checks the Content-Type, but we already check it ourselves,
        // so we override the default 'type' parameter.
        type: () => true,
        xmlParseOptions: {
    ...
    return function (req, res, next) {
        function afterBody() {
            config
                ...
                .info(
                    context,
                    "*** received content type '%s', matched to '%s', body '%s'",
                    content_type,
                    matched_type,
                    req.body
                );
        ...
        // We cannot use req.is here because bodyparser middleware has
        // not been used yet, so this function can be called before the body
        // of the request has been read.
        // If the body has not been read yet, req.is will return null
        // even inf the Content-Type header is present and valid.
        // See https://expressjs.com/en/api.html#req.is
        const content_type = (req.get('content-type') || 'application/json').split(';').shift();
        let matched_type = 'no matched type';
        if (typeis(content_type, ['text/plain'])) {
            matched_type = 'text/plain';
            textParser(req, res, afterBody);
        } else if (typeis(content_type, ['application/octet-stream'])) {
            matched_type = 'application/octet-stream';
            rawParser(req, res, afterBody);
        } else if (typeis(content_type, ['application/soap+xml'])) {
            matched_type = 'application/soap+xml';
            xmlParser(req, res, afterBody);
        } else {
            // req.is('json')
            matched_type = 'application/json';
            jsonParser(req, res, afterBody);
        }
    };
}

function checkContentType(...supportedTypes) {
    return function (req, res, next) {
        const content_type = (req.get('content-type') || 'undefined').split(';').shift();
        let error;
        if (!typeis(content_type, supportedTypes)) {
            error = new errors.UnsupportedType(supportedTypes);
        }
        next(error);
    };
}
...
function start(callback) {
    ...
    httpBindingServer.router.post(
        (config.getConfig().iota.defaultResource || constants.HTTP_MEASURE_PATH) +
            ...
            '/:attrValue',
        checkContentType('json', 'text/plain', 'application/octet-stream', 'application/soap+xml'),
        parserBody(),
        checkMandatoryParams(false),
