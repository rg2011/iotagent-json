...
class EmptyPayload {
    constructor(payload) {
        this.name = 'EMPTY_PAYLOAD';
        this.message = 'The request payload must not be empty';
        this.code = 400;
    }
}
class BadPayload {
...
class HTTPCommandResponseError {
    constructor(code, error) {
        ...
        this.message = 'There was an error in the response of a device to a command [' + code + ' ]:' + JSON.stringify(error);
        this.code = 400;
    }
}
...
class DeviceEndpointError {
    constructor(code, msg) {
        ...
        this.message = 'Request to the device ended up in error with code [' + code + ' ] and message [' + JSON.stringify(msg) + ']';
        this.code = code;
    }
}
